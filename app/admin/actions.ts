...
export async function updateTeamApplicationAction(formData: FormData) {
  ...
  const reviewedFields =
    status === "approved" || status === "invite_sent" || status === "active" || status === "rejected"
...
async function activateTeamApplicationById(
  ...
  try {
    ...
    const salesRepPayload = {
      user_id: profile.id,
      display_name: profile.display_name || application.name || profile.email || "Sales Rep",
      active: true,
    };
...
export async function markTeamInviteSentAction(formData: FormData) {
  ...
  revalidatePath("/admin");
  return success("Invite marked as sent.");
}
